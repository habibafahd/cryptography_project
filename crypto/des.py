...
def left_shift(lst,n): return lst[n:] + lst[:n]
...
def des_block(block_bits, subkeys, encrypt=True):
    ...
    round_keys = subkeys if encrypt else subkeys[::-1]
    for rk in round_keys:
        L, R = R, xor(L, feistel(R, rk))
    return permute(R+L, FP)

def int_to_bytes(key_int):
    return [(key_int >> (8*(7-i))) & 0xFF for i in range(8)]

def encrypt(msg_bytes, key_int):
    key_bytes = bytes(int_to_bytes(key_int))
    subkeys = generate_subkeys(bytes_to_bits(key_bytes))
    if len(msg_bytes) % 8 != 0:
        msg_bytes += b'\0' * (8 - len(msg_bytes) % 8)
    bits = []
    for i in range(0, len(msg_bytes), 8):
        block_bits = bytes_to_bits(msg_bytes[i:i+8])
        bits += des_block(block_bits, subkeys, encrypt=True)
    return bits_to_bytes(bits)

def decrypt(ciphertext, key_int):
    key_bytes = bytes(int_to_bytes(key_int))
    subkeys = generate_subkeys(bytes_to_bits(key_bytes))
    bits = []
    for i in range(0, len(ciphertext), 8):
        block_bits = bytes_to_bits(ciphertext[i:i+8])
        bits += des_block(block_bits, subkeys, encrypt=False)
    return bits_to_bytes(bits).rstrip(b'\0')
